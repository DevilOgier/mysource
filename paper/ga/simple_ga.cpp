...
//
# define POPSIZE 50 //个体数目
# define MAXGENS 1000  //迭代的次数
# define NVARS 3 //个体的长度，也就是变量的个数
# define PXOVER 0.8 //杂交的概率
# define PMUTATION 0.15 //变异的概率
//
...
struct genotype
{
  double gene[NVARS]; // 坐标值
  double fitness;  //适应值
  double upper[NVARS]; // 变量对应的上界
  double lower[NVARS]; // 变量对应的下届
  double rfitness; //相对适应值比例
  double cfitness; //相对适应值比例之和
};

struct genotype population[POPSIZE+1]; //种群 （+1，最后一个用来存放best）
