...
generate_new_pop() {
	...
	for (k=0; k<= pop_size-1; k +=2) {
		...
		case UNIF :
			...
			break;
		case ONLINE:
			cross_over_line(mate1,mate2,k,k+1);
			break;
		}
		mutation(&newpop[k]);
		mutation(&newpop[k+1]);
...
FILE *fp;
{
	...
	switch (s_strategy) {
	case 1 :
		fprintf(fp,"\n Tournament Selection Used (Size = %d)",tourneysize);
		break;
	case 2 :
		fprintf(fp,"\n Roulette Wheel Selection Used");
		break;
	case 3 :
		fprintf(fp,"\n Stochastic Remainder RW Selection Used");
		break;
	}
	switch (x_strategy) {
	...
	case UNIF  :
		...
		break;
	case ONLINE :
		fprintf(fp,"\n Crossover Strategy : On a straight line");
		break;
	default   :
		fprintf(fp,"\n CROSSOVER NOT SET CORRECTLY ");
		break;
	}
	if (BINGA)
