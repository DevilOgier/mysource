...
const int maxCycle=1000000;//停止条件
...
int main()
{
    ...
    while(gen<=maxCycle)
    {
        ...
        if (gen%100 ==0){
        output<<"count:"<<gen<<" "<<setprecision(30)<<BestSource.trueFit<<" x1: "<<BestSource.code[0]<<" x2: "<<BestSource.code[0]<<endl;
        }
...
void CalculateProbabilities()//计算轮盘赌的选择概率,先把fitness归一化，得到rfitness
{
    int i;
    double maxfit;
