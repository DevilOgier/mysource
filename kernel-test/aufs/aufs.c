...
struct file_operations aufs_file_operations = {
	.read =		default_read_file,
	.write =	default_write_file,
	.open =		default_open,
};*/
struct address_space_operations aufs_aops = {
...
static struct backing_dev_info aufs_backing_dev_info = {
...
static int __init aufs_init(void)
{
 ...
 //          nr_free_pagecache_pages();
   ...
   /*     printk("total pages:%d\n",nr_free_pagecache_pages());
	...
	pslot = aufs_create_file("slot4", S_IFREG | S_IRUGO, NULL, NULL, NULL);
	if(!pslot) 
		 printk(KERN_WARNING "aufs_create_file() failed!");

	pslot = aufs_create_file("slot5", S_IFREG | S_IRUGO, NULL, NULL, NULL);
	if(!pslot) 
		 printk(KERN_WARNING "aufs_create_file() failed!");

	pslot = aufs_create_file("slot6", S_IFREG | S_IRUGO, NULL, NULL, NULL);
	if(!pslot) 
		 printk(KERN_WARNING "aufs_create_file() failed!");

pslot = aufs_create_file("slot7", S_IFREG | S_IRUGO, NULL, NULL, NULL);
	if(!pslot) 
		 printk(KERN_WARNING "aufs_create_file() failed!");

	pslot = aufs_create_file("slot8", S_IFREG | S_IRUGO, NULL, NULL, NULL);
	if(!pslot) 
		 printk(KERN_WARNING "aufs_create_file() failed!");

	pslot = aufs_create_file("slot9", S_IFREG | S_IRUGO, NULL, NULL, NULL);
	if(!pslot) 
		 printk(KERN_WARNING "aufs_create_file() failed!");

	pslot = aufs_create_file("slot10", S_IFREG | S_IRUGO, NULL, NULL, NULL);
	if(!pslot) 
		 printk(KERN_WARNING "aufs_create_file() failed!");
	return retval;
}
